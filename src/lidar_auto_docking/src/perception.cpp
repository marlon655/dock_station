...
DockPerception::DockPerception(std::shared_ptr<rclcpp::Node> node_ptr)
    : listener_(node_ptr->get_clock()),
      ...
      found_dock_(false),
      scan_rotation_logged_(false) {
  node_ptr_ = node_ptr;
  debug_ = false;

  cluster_tolerance_ =
      node_ptr_->declare_parameter<double>("cluster_tolerance", 0.05);
  scan_rotation_deg_ =
      node_ptr_->declare_parameter<double>("scan_rotation_deg", 180.0);
  debug_clusters_ =
      node_ptr_->declare_parameter<bool>("debug_clusters", false);
  RCLCPP_INFO(node_ptr_->get_logger(), "Cluster tolerance: %.3f m",
              cluster_tolerance_);
  RCLCPP_INFO(node_ptr_->get_logger(), "Scan rotation: %.3f deg",
              scan_rotation_deg_);
  RCLCPP_INFO(node_ptr_->get_logger(), "Cluster debug logs: %s",
              debug_clusters_ ? "enabled" : "disabled");
  ...
  */
  ...
  // Volatile durability is intentional: consumers must not receive an old
  // detection as if it were current. They must also validate header.stamp.
  dock_detection_pub_ = node_ptr_->create_publisher<
      lidar_auto_docking_messages::msg::DockDetection>(
      "/dock_detection_info", rclcpp::QoS(rclcpp::KeepLast(1)).reliable());
...
void DockPerception::callback(
  ...
  // splitConnected() treats a LaserScan as a linear sequence. On a 360-degree
  // lidar whose forward direction is the last-index -> index-0 seam, a dock
  // straight ahead is incorrectly split in two. Rotate only this private copy
  // so the seam moves behind the robot; the subscribed /scan is not changed or
  // republished, and ScanProcessor still sees the same physical points.
  const lidar_auto_docking::ScanRotationResult rotated_scan =
      lidar_auto_docking::rotateLaserScan(*scan, scan_rotation_deg_);

  if (!scan_rotation_logged_) {
    if (!rotated_scan.full_circle && scan_rotation_deg_ != 0.0) {
      RCLCPP_WARN(node_ptr_->get_logger(),
                  "Scan rotation skipped: scan is empty, invalid, or not a "
                  "complete 360-degree scan");
    }
    RCLCPP_INFO(node_ptr_->get_logger(),
                "Scan rotation shift: %lld samples | input=%zu processed=%zu "
                "| duplicate endpoint removed=%s",
                static_cast<long long>(rotated_scan.shift_samples),
                scan->ranges.size(), rotated_scan.scan.ranges.size(),
                rotated_scan.duplicate_endpoint_removed ? "yes" : "no");
    scan_rotation_logged_ = true;
  }

  // Cluster the local, rotated laser scan.
  laser_processor::ScanMask mask;
  laser_processor::ScanProcessor processor(rotated_scan.scan, mask);
  processor.splitConnected(cluster_tolerance_);
  ...
  for (std::list<laser_processor::SampleSet*>::iterator i =
       ...
       i != processor.getClusters().end(); ++i, ++cluster_id) {
    laser_processor::SampleSet* cluster = *i;

    DockCandidatePtr c = extract(cluster);

    if (!c || c->points.empty()) {
      RCLCPP_WARN(node_ptr_->get_logger(),
                  "Cluster %d: vazio ou falha no TF", cluster_id);

      continue;
    }

    const double width = c->width();
    const bool valid = c->valid(found_dock_);

    if (debug_clusters_) {
      auto first_sample = *cluster->begin();
      auto last_iterator = cluster->end();
      --last_iterator;
      auto last_sample = *last_iterator;

      RCLCPP_WARN(node_ptr_->get_logger(),
                  "Cluster %d | samples=%zu | "
                  "indexes=[%d, %d] | "
                  "width=%.3f | dist2=%.3f | valid=%s",
                  cluster_id, cluster->size(), first_sample->index,
                  last_sample->index, width, c->dist,
                  valid ? "SIM" : "NAO");
    }
  ...
  std::uint32_t best_point_count = 0;
  double best_icp_fitness = -1.0;
  while (!candidates.empty()) {
    geometry_msgs::msg::Pose pose = dock_.pose;
    const DockCandidatePtr candidate = candidates.top();
    // fit() replaces candidate->points with the transformed ideal cloud, so
    // preserve the observed candidate geometry before invoking it.
    const double candidate_width = candidate->width();
    const std::uint32_t candidate_point_count =
        static_cast<std::uint32_t>(candidate->points.size());
    double icp_fitness = -1.0;
    double score = fit(candidate, pose, icp_fitness);
    if (score >= 0) {
      best = candidate;
      best_pose = pose;
      best_width = candidate_width;
      best_point_count = candidate_point_count;
...
*/
  ...
  // Publish only after the selected candidate has passed ICP, pose-jump
  // validation, and filtering. This informational pose is transformed back to
  // the LaserScan frame so lidar_distance is the physical planar distance from
  // the LiDAR. The internal dock pose used by docking remains unchanged in
  // tracking_frame_ (currently odom).
  if (scan->header.frame_id.empty()) {
    RCLCPP_ERROR(node_ptr_->get_logger(),
                 "Cannot publish dock detection with an empty scan frame.");
  } else {
    try {
      geometry_msgs::msg::PoseStamped lidar_pose;
      listener_.transformPose(scan->header.frame_id, dock_, lidar_pose);
      // transformPose() uses the TF header. Restore the originating scan stamp
      // required by DockDetection consumers.
      lidar_pose.header.stamp = scan->header.stamp;
      lidar_pose.header.frame_id = scan->header.frame_id;
      dock_detection_pub_->publish(lidar_auto_docking::makeDockDetection(
          lidar_pose.header, lidar_pose.pose, best_width, best_point_count,
          best_icp_fitness));
    } catch (const tf2::TransformException& error) {
      RCLCPP_ERROR(node_ptr_->get_logger(),
                   "Cannot transform accepted dock pose to %s: %s",
                   scan->header.frame_id.c_str(), error.what());
    }
  }
}
...
double DockPerception::fit(const DockCandidatePtr& candidate,
  ...
  if (fitness >= 0.0) {
    ...
    if (debug_clusters_) {
      ...
      const auto& first = candidate->points.front();
      const auto& last = candidate->points.back();

      RCLCPP_WARN(
          node_ptr_->get_logger(),
          "width()=%.3f points=%zu | "
          "first=(%.3f, %.3f) last=(%.3f, %.3f) | "
          "dx=%.3f dy=%.3f",
          candidate->width(), candidate->points.size(), first.x, first.y,
          last.x, last.y, max_x - min_x, max_y - min_y);
    }
