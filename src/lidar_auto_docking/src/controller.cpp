...
BaseController::BaseController(std::shared_ptr<rclcpp::Node> node_ptr)
  ...
  beta_ = 0.2;
  lambda_ = 2.0;
  node_ptr->declare_parameter<double>("dock_lateral_offset", -0.04);
  node_ptr->get_parameter("dock_lateral_offset", dock_lateral_offset_);
  node_ptr->declare_parameter<double>("dock_yaw_offset", 0.03);
  node_ptr->get_parameter("dock_yaw_offset", dock_yaw_offset_);
}
...
bool BaseController::approach(const geometry_msgs::msg::PoseStamped& target) {
  ...
  geometry_msgs::msg::PoseStamped pose = target;
  {
    double theta = angles::normalize_angle(
        tf2::getYaw(pose.pose.orientation) + dock_yaw_offset_);
    ...
    pose.pose.position.x +=
        cos(theta) * -dist_ - sin(theta) * dock_lateral_offset_;
    pose.pose.position.y +=
        sin(theta) * -dist_ + cos(theta) * dock_lateral_offset_;
  }
