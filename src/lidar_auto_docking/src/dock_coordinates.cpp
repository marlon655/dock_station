...
public:
  DockCoordinates() : Node("dock_coordinates"), tf2_listen(this->get_clock())
  {
    ...
    this->declare_parameter<double>("initial_dock_distance", 1.5);
    this->get_parameter("initial_dock_distance", initial_dock_distance);
  ...
  // this function updates the initial dock pose to be a configurable distance
  // in front of the robot wrt map frame.
  // wrt map frame.
  void update_init_dock(geometry_msgs::msg::PoseStamped &idp)
  {
    ...
    fake_dock.pose.position.x = initial_dock_distance;
