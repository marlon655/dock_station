...
class dock_pose_subscriber(Node):
    ...
    async def on_timer(self):
        ...
        try:
            ...
            self.bot_yaw = 2.0 * math.atan2(self.bot_z, self.bot_w)
            self.dock_yaw = 2.0 * math.atan2(self.z_pos, self.w_pos)
            ...
            self.obj_gui_.show_variable(
                ...
                msg=(
                    "Dock map pose: "
                    f"x={self.x_pos:.3f}, y={self.y_pos:.3f}, yaw={self.dock_yaw:.4f}"
                ),
            )
            self.obj_gui_.show_variable(
                posx=60,
                ...
                msg=(
                    "Robot map pose: "
                    f"x={self.bot_x:.3f}, y={self.bot_y:.3f}, yaw={self.bot_yaw:.4f}"
                ),
            )
    ...
    def save_dock_callback(self):
        ...
        print(
            "Saved dock pose: "
            f"x={self.x_pos:.3f}, y={self.y_pos:.3f}, yaw={2.0 * math.atan2(self.z_pos, self.w_pos):.4f}"
        )
        print(
            "Saved robot pose: "
            f"x={self.bot_x:.3f}, y={self.bot_y:.3f}, yaw={2.0 * math.atan2(self.bot_z, self.bot_w):.4f}"
        )
        with open(
            self.dock_file_path,
            "w",
