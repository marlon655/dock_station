...
class DockPerception {
 ...
 private:
  ...
  rclcpp::Publisher<
      lidar_auto_docking_messages::msg::DockDetection>::SharedPtr
      dock_detection_pub_;
  ...
  // Maximum distance between adjacent scan points in the same cluster
  double cluster_tolerance_;
  // Circular shift applied only to the private scan used for dock perception
  double scan_rotation_deg_;
  // Enable per-scan cluster and candidate geometry logs
  bool debug_clusters_;
  // The sample shift depends on scan size/increment, so log it on first scan
  bool scan_rotation_logged_;
